...
func (h *Handler) StreamScanHandler(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.scanner.ScanStream(r.Body)
	if err != nil {
...
// MultiScanHandler 处理多文件扫描请求
func (h *Handler) MultiScanHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, "只支持POST方法", http.StatusMethodNotAllowed)
		return
	}
	...
	for _, fileHeader := range files {
		tempFile, err := saveUploadedFile(fileHeader, h.config.TempDir)
		if err != nil {
			http.Error(w, fmt.Sprintf("保存文件失败: %v", err), http.StatusInternalServerError)
			return
		}
	...
	results, err := h.scanner.MultiScan(filePaths)
	if err != nil {
		http.Error(w, fmt.Sprintf("多文件扫描失败: %v", err), http.StatusInternalServerError)
		return
	}
